...
- 只要功能存在玩家可调行为（开关、频率、阈值、强度、预设、快捷键等），就必须提供 MCM 入口，且默认值 fail-safe。
- 如果功能不需要 MCM，必须在 PLAN 中写明理由（例如纯内部管道、无玩家可调参数）。
- MCM 改动必须同步检查菜单分组、中英文、预设联动、`Config.json` 兼容与开发者调试入口是否分开。

## 任务连续性

- 遵循已安装 skill `awake-task-continuity`：先读任务队列，续旧任务，再处理新想法。
- 每次开始新请求前先读 `docs/AWAKE-Task-Queue-20260816.md`。
- 先继续进行中任务；新想法、新建议先登记到任务队列，不顶替当前任务。
- 当前任务完成后或没有明确任务时，必须列出延续候选并等待用户决策，不自动选择下一项。
