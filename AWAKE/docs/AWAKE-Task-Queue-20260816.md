...
> 当前状态：UI 审计与场景崩溃防护代码完成；场景可视化选人待拍板，未提版。

## 当前检查点

- 当前任务：场景可视化选人（decision_needed）。
- 最近完成：UI 排版与本地化审计；场景原生回退崩溃防护；overlay 焦点不再被即时状态误判。
- 下一步：确认 AF 式可视化选人的具体形态（地面扇形/候选高亮/简化版），走 grill-me 锁定 PLAN 后实施。
- 阻塞：游戏内验收需用户运行游戏。
...
9. 场景可视化选人：AF 式地面扇形范围 + 候选高亮（金色/主目标品红），替代纯 Y 循环；与 BatchA“不做大量地面预览”冲突，需拍板是否采用简化版。
