...
- 标记必须非常明显：地面范围、候选高亮、当前目标高亮三者要能一眼分辨。
- 地面扇形不得与环境色接近：采用高饱和、带轮廓/光效/动画呼吸的标记，禁用只改 alpha 或只靠小圆点的实现。
- 候选 NPC 高亮必须覆盖全身轮廓，颜色与当前目标区分明显（候选金色、当前目标品红），并保证最大搜索距离下仍可见。
- 远距离时标记必须可缩放/可加高，不能因为距离变远而退化成看不清的小点；若真实实体 spawn 无法满足可见性，允许用屏幕空间提示辅助（方向箭头 + 距离标尺），不能静默降级到不可见。
- 验收时至少覆盖：近距离多目标、最远距离、室内/室外、昼夜光照差异；任一场合看不清即不通过。
