...
- 远程写信直接整合进通讯录，不单独做“信件面板”
- 远方联系人会话窗口内切换“写信模式”：收件人、正文、费用预览、发送
- 来信也进入通讯录联系人列表：NPC 来信显示未读标记，点击后直接在会话窗口阅读和回复
- 距离计费、回复延迟、NPC 主动来信
- 地图通知只负责提醒“有新来信”，具体阅读/回复仍在通讯录内完成
...
- 所有与 NPC 的异步通信统一走通讯录，不再出现独立的“写信弹窗/收件箱面板”。
- 遭遇面谈、场景内 T/Y、通讯录深谈只是不同入口，最终都进入同一套会话视图。
- 来信、写信、未读、历史、回复全部挂在同一个联系人会话上。
- 地图通知只做轻提示，不承载阅读和操作。

> 注意：这里的“同一套会话视图”只指远程/异步通信统一进通讯录；原版对话 UI 与场景内 AI 对话覆盖层不并入 Messenger，也不被替换。
