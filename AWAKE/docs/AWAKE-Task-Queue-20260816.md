...
> 当前状态：Messenger 持久化完成；新增“马库斯链接方式转接”排队想法，等待用户选择，未提版。
...
- 世界事件 / 政令。
- 内容包接入后的事件内容批次。
- 马库斯框架链接方式转接到 AWAKE：自动检测/引导 Marcus 安装、游戏内一键同步路由与 Provider、启动向导，降低玩家操作。待评估是否与 Marcus 权限/架构冲突，且只做引导不复制框架。
